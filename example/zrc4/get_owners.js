...
async function main() {
    try {
        ...
        console.log(`${address}`);
        console.log("------------------------ begin get owners ------------------------\n");

        const contract =  zilliqa.contracts.at(MULTISIG_CONTRACT_ADDR);
        const state = await contract.getState();

        if (Object.keys(state.owners).length === 0) {
            console.error("Owners list is empty");
        } else {
            console.log("Wallet: %o", MULTISIG_CONTRACT_ADDR);
            console.log("Owners: %o", Object.keys(state.owners));
        }
        
    } catch (err) {
        console.log(err);
    }
    console.log("------------------------ end get owners ------------------------\n");
}
